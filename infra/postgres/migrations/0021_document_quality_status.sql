-- One auditable quality decision per active document.  Individual verifier
-- runs remain append-only; this view selects only the newest result from each
-- verifier family and makes missing evidence explicit instead of treating it
-- as success.
BEGIN;
...
SELECT d.id AS document_id,
  ...
  CROSS JOIN LATERAL (
    SELECT d.lane = 'E4' OR EXISTS (
      SELECT 1 FROM page p WHERE p.document_id=d.id AND p.lane='E4'
    ) AS required
  ) uses_ocr
  LEFT JOIN LATERAL (
...
COMMENT ON VIEW v_document_quality_status IS
  'Latest independent character, OCR, sequence, and language-quality evidence for each active document; missing evidence is never a pass.';
